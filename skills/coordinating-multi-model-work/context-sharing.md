...
2. CP0 MUST run stellaris via `search_code` to retrieve the minimum current local code context needed for routing before CP1 on every task, including trivial/current-file edits. Use `get_file_outline`, `get_file_folded`, and `get_symbol` for token-efficient drill-down after initial search (AST-aware, voyage-code-3 embeddings, LanceDB + FTS5 + RRF fusion).
3. If `stellaris search_code` errors, is unavailable, permission-blocked, or returns tool failure, CP0 fail-closes with `BLOCKED` and stops before CP1; do not switch to file tools, Grok Search, or executors.
