...
3. **Load resume artifacts (if present)** — read `<plan-dir>/.handover.md` for current phase, next action, and cached worker `SESSION_ID`s (`session_refs` frontmatter), then `PHASE-<N>.md` from `read_first`. Skip if flat single-file plan with no folder.
