...
Before the first executor call, do minimal CP0 context acquisition: decide if selective `docs/wiki/` durable knowledge lookup is useful, use it only for complex planning/architecture/debugging/refactors or prompts asking what was known/decided/tried, then MUST run context-retrieval via `codebase-retrieval` for current local context before CP1 on every task (no trivial/current-file skip). Optionally run `stellaris search_code` in parallel as secondary source for code-specific symbol-level precision. If `codebase-retrieval` errors, is unavailable, permission-blocked, or returns tool failure, immediately output `BLOCKED` and stop before CP1. Do not switch to file tools, Grok Search, or executors. Stellaris failure does NOT trigger `BLOCKED`. Use Grok Search only for external/current research after mandatory local retrieval succeeds.
