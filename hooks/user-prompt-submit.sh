...
Immediately after CP0 completes, run CP1 Phase Assessment & Routing using the original user request and the CP0 context artifacts, then build one phase-scoped context bundle for the next implementation phase.
CP1 routing guide:
...
If CP1 routes to Gemini, Codex, or Cross-Validation, run CP2 External Execution using a phase-scoped context bundle: compressed original request, context refs, hydrated context snippets, and the CP1 phase summary plus success criteria, reviewer checklist, and integration checks. Reuse the same worker SESSION_ID only for follow-up fixes on that phase, and send deltas only on follow-up turns. Require final file contents directly (preferred) or a unified diff patch using External Response Protocol v1.1.
Smart context budget: planner phase context <= 1500 tokens, executor prompt context <= 2500 tokens, HYDRATED_CONTEXT <= 800 tokens and preferably <= 300 tokens, same-phase follow-up <= 1000 tokens. If over budget, narrow the phase or replace snippets with CONTEXT_REFS. Never pre-write new implementation inside HYDRATED_CONTEXT.
