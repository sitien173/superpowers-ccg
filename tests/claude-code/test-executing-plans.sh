...
output=$(run_claude "What is the superpowers-ccg:executing-plans skill? Describe its key steps briefly." 60)
assert_contains "$output" "executing-plans|Executing Plans|phase execution|current phase|active phase|implementation phases" "Skill is recognized" || exit 1
...
output=$(run_claude "In the executing-plans skill, what is the review step: CP4 phase review, or Sonnet code quality review?" 60)
...
output=$(run_claude "How should executing-plans assign implementation work: one primary executor per phase, or multiple workers on the same implementation phase?" 60)
...
output=$(run_claude "Does executing-plans say to reuse the same worker SESSION_ID for fixes on the same phase?" 60)
assert_contains "$output" "same worker|SESSION_ID|reuse" "Session reuse mentioned" || exit 1
echo ""

echo "Test 5: No prototype-then-rewrite..."
output=$(run_claude "In executing-plans, should Claude ask for a prototype/reference and then rewrite it later?" 60)
...
output=$(run_claude "In executing-plans, how does the worker return its work: by pasting a prose prototype, or by editing files directly via MCP write tools and reporting via External Response Protocol v1.1?" 60)
...
output=$(run_claude "When should executing-plans use cross-validation?" 60)
